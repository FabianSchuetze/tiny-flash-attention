...
template <class ElementType, class SmemLayoutQO, class SmemLayoutK,
          class SmemLayoutV>
...
__global__ void naive_nrow_gemm(FP *A, FP *B, FP *C, FP a, FP b, int M, int N,
                                int K, int mBlock);
__global__ void causal_mask_qk(FP *qk, int m);
__global__ void row_softmax(FP *input, FP *output, int n);
__global__ void naive_pv(FP *P, FP *V, FP *O, int M, int N, int mBlock);
...
template <int kBlockM, int kBlockN, int kNWarps, typename Engine,
          typename Layout>
inline __device__ void mask_within_nblock(Tensor<Engine, Layout> &tensor,
    ...
    const int row_idx_offset =
        kBlockM * m_block + lane_id / 4 + nrow_group * 16 /* 2*8 */;
    // (2, nrow), 2*8 for each
...
#pragma unroll
                for (int mj = 0; mj < size<0, 1>(tensor); ++mj) {
                    const int row_idx =
                        row_idx_offset + mi * 8 + mj * group_stride;
                    if (col_idx > row_idx) {
                        tensor(make_coord(mi, mj), make_coord(j, nj)) =
                            -INFINITY;
                    }
                }
            }
...
template <typename Tensor0, typename Tensor1, typename Tensor2,
          ...
          typename ThrCopy>
inline __device__ void gemm_A_in_regs(Tensor0 &acc, Tensor1 &tCrA,
...
// TODO: 原版处这里identity_MN是用来跳过大块的block的,
// predicate用于跳过block内的拷贝
...
inline __device__ auto convert_layout_rowcol_Aregs(Layout rowcol_layout) {
    ...
    auto l = logical_divide(
        rowcol_layout,
        Shape<X, Shape<X, Int<MMA_N_divisor>>>{});  // ((2, MMA_M), (2, (2,
                                                    // MMA_N / 2)))
...
#pragma unroll
    for (int mi = 0; mi < size<0>(tensor); ++mi) {
        // If max is -inf, then all elements must have been -inf (possibly due
        // to masking). We don't want (-inf - (-inf)) since that would give NaN.
        // If we don't have float around M_LOG2E the multiplication is done in
        // fp64.
...
// MMA_N))
// TODO: 搞清楚经过convert_layout_acc_rowcol后(nrow=(2, MMA_M), ncol=(2,
// MMA_N))的数学含义 形象的解释是把
//    T1.V0
//    T1.V1
//    T1.V0
...
inline __device__ auto convert_layout_acc_rowcol(Layout acc_layout) {
    ...
    // TD [2023-08-13]: Idk why but get<0, 1>(l) doesn't work for Cutlass 3.2,
    // I'm getting "int_tuple.hpp(74): error: conversion to inaccessible base
    // class" return make_layout(make_layout(get<0, 1>(l), get<1>(l)),
...
inline __device__ void softmax_rescale_o(Tensor0 &scores, Tensor1 &scores_max,
    ...
    if (Is_first) {
        ...
        // Reshape acc_o from (MMA=4, MMA_M, MMA_K) to (nrow=(2, MMA_M),
        // ncol=(2, MMA_K)) 将acc_o转换成符合2D直觉的(nrow, ncol)的形状
...
__global__ void flash_attention_v2_cutlass_kernel(const Params params) {
    ...
    using SharedStorage =
        SharedStorage<Element, SmemLayoutQ, SmemLayoutK, SmemLayoutV>;
    ...
    Tensor gQ = local_tile(Q, make_tile(Int<kBlockM>{}, Int<kHeadDim>{}),
                           make_coord(m_block, _));

    // (kBlockN, kHeadDim, num_tile_n)
    // NOTE: loading流水线, 初次加载所需K, V
    Tensor gK = local_tile(K, make_tile(Int<kBlockN>{}, Int<kHeadDim>{}),
                           make_coord(0, _));
    Tensor gV = local_tile(V, make_tile(Int<kBlockN>{}, Int<kHeadDim>{}),
                           make_coord(0, _));
    ...
    Tensor sVtNoSwizzle = make_tensor(
        make_smem_ptr(shared_storage.smem_v.data()), SmemLayoutVtNoSwizzle{});
    ...
    int n_block_max = Is_causal ? cute::ceil_div(seqlen_end, kBlockN)
                                : cute::ceil_div(params.seqlen, kBlockN);

    // NOTE: 需要记录的max
    Tensor scores_max =
        make_tensor<ElementAccum>(Shape<Int<2 * size<1>(rAccOut)>>{});
    // NOTE: 需要记录的denom
    ...
    for (int nbi = n_block_min; nbi < n_block_max; nbi++) {
        ...
        nbi == 0
            ...
            : flash::softmax_rescale_o</*Is_first=*/false>(
                  scores, scores_max, scores_sum, rAccOut,
    ...
    // MMA_K))
    Tensor acc_o_rowcol = make_tensor(
        rAccOut.data(), flash::convert_layout_acc_rowcol(rAccOut.layout()));
// for row
#pragma unroll
...
#pragma unroll
    ...
    Tensor sO = make_tensor(
        sQ.data(), typename Kernel_traits::SmemLayoutQO{});  // (SMEM_M,SMEM_N)
    ...
    // Use ElementAccum(f32) to debug
        ...
        make_shape(params.seqlen, params.dim),
        make_stride(params.dim, Int<1>{}));
    Tensor gO = local_tile(O, make_tile(Int<kBlockM>{}, Int<kHeadDim>{}),
                           make_coord(m_block, _));
...
void flash_attention_v2_cuda(FPC *Q, FPC *K, FPC *V, FPC_O *O, int bs, int head,
                             int seqlen, int dim) {
    ...
    constexpr size_t smem_size = size_t(
        sizeof(SharedStorage<Element, SmemLayoutQ, SmemLayoutK, SmemLayoutV>));
    ...
    auto kernel = &flash_attention_v2_cutlass_kernel<Kernel_traits, IS_CAUSAL,
                                                     Flash_fwd_params>;
    // NOTE: smem过大时需要设置
...
void self_attention_run(FP *Q, FP *K, FP *V, FP *O, int m, int n,
                        bool is_causal, float sm_scale = 1) {
    int mBlock = 2;
...
void self_attention_cuda(FP *Q, FP *K, FP *V, FP *O, int bs, int head,
                         int seqlen, int dim, bool is_causal,
                         float sm_scale = 1) {
...
void test_attention() {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        self_attention_cuda(d_Q, d_K, d_V, d_O, bs, head, m, n, is_causal,
                            softmax_scale);
    ...
    assert(all_close(h_O, h_O2, total_size) &&
...
int main() {
    int epoch = 1;
    for (int i = 0; i < epoch; i++) test_attention();
